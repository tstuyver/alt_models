...
parser.add_argument('--csv_file_train', type=str, required=False,
                    help='path to file containing training/validation rxn-smiles')
parser.add_argument('--csv_file_test', type=str, required=False,
...
parser.add_argument('--pkl_file_train', type=str, required=False,
                    help='path to file containing training/validation descriptor values')
parser.add_argument('--pkl_file_test', type=str, required=False,
                    help='path to file containing testing descriptor values')
parser.add_argument('--n_train', default=None, required=False,
                    help='number (or fraction) of training points')
parser.add_argument('--clustering_analysis', dest='clustering_analysis', action='store_true',
...
if __name__ == '__main__':
    ...
    if args.clustering_analysis:
        ...
        if args.selective:
            ...
            logger.info('Random Forest based on fingerprints (2,2048) -- results:')
            random_forest_fp_selective(args.csv_file_train, args.csv_file_test, logger, 2, 2048)
            logger.info('Random Forest based on fingerprints (2,4096) -- results:')
            random_forest_fp_selective(args.csv_file_train, args.csv_file_test, logger, 2, 4096)
            ...
            if args.n_train:
                ...
                n_train=0.8
            logger.info('Random Forest based on fingerprints (2,1024) -- results:')
            random_forest_fp(args.csv_file, logger, n_train)
            logger.info('Random Forest based on fingerprints (2,2048) -- results:')
            random_forest_fp(args.csv_file, logger, n_train, 2, 2048)
            logger.info('Random Forest based on fingerprints (2,4096) -- results:')
            random_forest_fp(args.csv_file, logger, n_train, 2, 4096)
            logger.info('Random Forest based on fingerprints (3,2048) -- results:')
            random_forest_fp(args.csv_file, logger, n_train, 3, 2048)
            logger.info('Random Forest based on descriptors -- results:')
            random_forest_descs(args.pkl_file, logger, n_train)
            logger.info('Linear model based on descriptors -- results:')
            linear_model_descs(args.pkl_file, logger)
            logger.info('SVR based on fingerprints -- results:')
            SVR_fp(args.csv_file, logger)
            #logger.info('SVR based on descriptors -- results:')
            #SVR_descs(args.pkl_file, logger)
